...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Executes the LangGraph app and yields its progress.
    It attempts to stream the graph's progress using stream_mode="updates" 
    (which yields the state updates from each node as they complete).
    If streaming fails, it falls back to a standard invoke.
    Yields tuples of ("updates"/"values"/"final", payload).
    """
    try:
        # stream_mode="updates" yields only the partial state modifications made by each node.
...
def extract_latest_state(current_state: Dict[str, Any], step_payload: Any) -> Dict[str, Any]:
    """
    Merges a partial state update (from a LangGraph node) into the current tracking state.
    Since LangGraph returns state updates in the format { "node_name": { "key": "value" } },
    this helper unpacks the inner dictionary to update the global `current_state`.
    """
...
def render_markdown_with_local_images(md: str):
    """
    Custom markdown renderer for Streamlit.
    Streamlit's native `st.markdown` doesn't support rendering local image files reliably.
    This function parses the markdown string, splits it into text and image blocks,
    and uses `st.image` to render images explicitly, ensuring they show up in the UI.
    """
...
# Sidebar for inputs and past blogs
with st.sidebar:
